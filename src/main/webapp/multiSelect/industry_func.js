...
var Industry = {
	...
	Show : function(){
		...
		output+="<p><h3>临床科室:</h3></p>";
		for (var i in ind_b){
			flag=in_array(i,ind_flag_arr)?' chkON':'';
			output+='<li class="Industry' + i + flag + '" onclick="Industry.Chk(\''+i+'\')">'+ind_b[i]+'</li>';
		}
		
		for (var i in ind_flag_arr){
			if(typeof(ind_a[ind_flag_arr[i]]) != "undefined"){
				output2+='<li class="Industry' + ind_flag_arr[i] + ' chkON" onclick="Industry.Chk(\''+ind_flag_arr[i]+'\')">'+ind_a[ind_flag_arr[i]]+'</li>';
			}
		}
		for (var i in ind_flag_arr){
			if(typeof(ind_b[ind_flag_arr[i]]) != "undefined"){
				output2+='<li class="Industry' + ind_flag_arr[i] + ' chkON" onclick="Industry.Chk(\''+ind_flag_arr[i]+'\')">'+ind_b[ind_flag_arr[i]]+'</li>';
			}
		}
		$('#drag').width('670px');
...
//			if(ind_flag_arr.length<5){
				...
				if(typeof(ind_a[id]) != "undefined"){
					var html='<li class="Industry'+id+'" onclick="Industry.Chk(\''+id+'\')">'+ind_a[id]+'</li>';
				}else if(typeof(ind_b[id]) != "undefined"){
					var html='<li class="Industry'+id+'" onclick="Industry.Chk(\''+id+'\')">'+ind_b[id]+'</li>';
				}
...
//			}
	...
	confirm : function(){
		...
		for(var i in ind_flag_arr){
			...
			}else if(typeof(ind_b[ind_flag_arr[i]]) != "undefined"){
				indStr+=','+ind_b[ind_flag_arr[i]];
			}
		}
		indStr=indStr.substring(1)?indStr.substring(1):'请选择留言对象';
...
function IndustrySelect(){
	var dragHtml ='<div id="IndustryAlpha">';		//行业
		dragHtml+='		<dl id="IndustrySelected"><dt>已选收信人：</dt><dd></dd></dl>';
		dragHtml+='		<div id="IndustryList"></div>';//行业列表
		dragHtml+='</div>';
	$('#drag_h').html('<b>请选择留言对象</b><span onclick="Industry.confirm()">确定</span>');
